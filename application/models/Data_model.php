...
class Data_model extends CI_Model {
    ...
    public function createData($form_data){
        $this->db->insert('data_alternatif', $form_data);
        return ($this->db->affected_rows() != 1) ? false : true;
    }

    public function updateData($form_data){
        $this->db->where('id', $form_data['id']);
        $this->db->update('data_alternatif', $form_data);
        return ($this->db->affected_rows() != 1) ? false : true;
    }
    ...
    public function deleteData($id){
        ...
        $this->db->where('id', $id);
        $this->db->where('cond', 1);
        $this->db->update('data_alternatif');
        return ($this->db->affected_rows() != 1) ? false : true;
    }
}